...
class collector_api():
    ...
    def variable_setting(self):
        ...
        # 봉: 아래 library로 들어가서 상장된 모든 종목의 이름과 code를 df로 가지고 있는다.
        ...
        # 봉: 아래 library로 들어가서 daily_buy_list와 daily_craw DB를 만들 준비를 한다.(create engine 하고 listen한다.여기는 잘 이해 안간다)
    ...
    def code_update_check(self):
        ...
        if rows[0][0] != self.open_api.today:
            ...
            # BONG: CHAPTER10-4 class에서 아래를 추가하였다. 매일매일 stock_info table을 update 하는 code
            self._create_stock_info()
            # BONG: 원래 아래 code는 위의 self.get_code_list() 안에 들어가면 가장 마지막에 있었는데 잘라와서 이 아래에 붙였다. setting_data를 마지막으로 update 하기 위해서
        ...
        print("BBONG!!: daily_buy_list와 daily_crawler는 모두 작업한 듯")
        ...
        #bong: chapter13-4 class에서 이 아랫부분을 추가하라고 해서 추가하는 거임. 'stock_finance 테이블 만드는 방법.txt'에서 가지고 옴
        #collector가 돌 때마다 stock_finance table을 만든단다.
        ...
        na.send_email(na.smtp_info, na.msg_4)
    ...
    def set_daily_crawler_table(self, code, code_name):
        ...
        # bong: print(df) 하면 셋팅한 오늘날짜부터 과거의 data를 일자별로 ohlc, volume data df로 보여주더라
        if len(df) == 0:
            return 1
...
#bong: 아래의 code는 chapter10-4에서 나온 것으로 stock_finance 테이블 만드는 방법.txt file에서 code 복사한 것을 붙여 넣기 한 것이다.
